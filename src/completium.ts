...
/**
 *
 * @param path path of the contract source file to deploy
 * @param obj
 * @param originate
 * @returns
 */
async function deploy(path : string, obj : DeployArguments, originate = false) {
...
async function getContract(contract_id) {
  return new Promise(async (resolve, reject) => {
    ...
    sigs.forEach(sig => {
      ...
      with_default = with_default || id === 'default';
      if (id === "_set_now") {
